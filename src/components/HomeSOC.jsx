...
import { homeSOC } from '../data/content.js'
import Icon from './Icon.jsx'

export default function HomeSOC() {
  const { eyebrow, title, subtitle, overview, capabilities, architecture, repository, status } = homeSOC

  return (
    <section id="home-soc" className="section border-t border-slate-100 bg-slate-50 dark:border-ink-700 dark:bg-ink-900">
      <div className="container-px mx-auto max-w-6xl">
        ...
        <div className="mt-10">
          ...
          <div className="card flex flex-col items-stretch gap-3 p-5 sm:flex-row sm:items-center sm:gap-3 sm:p-6">
            {architecture.flow.map((node, i) => (
              <Fragment key={node}>
                <div className="rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-center font-mono text-xs font-medium sm:flex-1 sm:text-sm dark:border-ink-600 dark:bg-ink-800">
                  {node}
                </div>
                {i < architecture.flow.length - 1 && (
                  <Icon
                    name="arrowRight"
                    className="mx-auto h-4 w-4 shrink-0 rotate-90 text-signal-500 sm:rotate-0"
                  />
                )}
              </Fragment>
            ))}
        ...
        <div className="mt-10 grid grid-cols-1 gap-6 lg:grid-cols-2">
          <div className="card p-6 sm:p-7">
            ...
            <a href={repository.url} target="_blank" rel="noreferrer" className="btn-secondary mt-5 inline-flex">
              View on GitHub
